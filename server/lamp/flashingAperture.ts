...
export default class FlashingApertureAnimation
  implements LampAnimation<[ColorOption]>
{
  ...
  constructor() {
    ...
    for (let i = 0; i < this.NB_OF_SEGMENTS; i++) {
      this.lines.push(
        new Line(NB_LED, i * this.SEG_LENGTH, (i + 1) * this.SEG_LENGTH - 1)
      );
    }
  }
  ...
  public animate(t, display: Display, options) {
    ...
    if (step < this.FLASH_DUR) {
      ...
      for (let i = 0; i < this.lines.length; i++) {
        ...
        display.drawGradient(
          this.lines[i],
          this.currentColor.withOpacity(ratio * 2),
          this.currentColor.withOpacity(ratio)
        );
      }
    } else {
      // Off
