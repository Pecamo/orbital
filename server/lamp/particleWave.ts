...
export default class ParticleWaveAnimation
  implements LampAnimation<[ColorOption, ColorOption]>
{
  ...
  public animate(t, display, options) {
    ...
    for (let n = 0; n < this.PARTICLE_COUNT; n++) {
      ...
      display.drawLine(
        new Line(NB_LED, particleStart, particleStart + this.PARTICLE_SIZE),
        options[1]
      );
    }

    for (let n = 0; n < this.PARTICLE_COUNT; n++) {
      this.particlesPos[n] = normalize(
        Math.floor(this.particlesPos[n] + this.speed)
      );
      const particleStart = this.particlesPos[n];
      display.drawLine(
        new Line(NB_LED, particleStart, particleStart + this.PARTICLE_SIZE),
