...
export default class BlueFireAnimation
  implements LampAnimation<[NumberOption]>
{
  ...
  public animate(t, display, options) {
    ...
    if (!this.rotation) {
      display.drawDot(
        ...
        this.swap(temperatureToRgb(this.temperatures[0]))
      );
      display.drawDot(
        normalize(NB_LED / 2 + topLedNb - 1),
        this.swap(temperatureToRgb(this.temperatures[NB_LED]))
      );
    } else {
      display.drawDot(
      ...
      const colorFront = this.swap(
        temperatureToRgb(this.temperatures[0] - 500)
      );
