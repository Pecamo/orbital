...
export function initLamp() {
  ...
  lamp.get("/animationNames", (req, res) => {
    res.send({
      animationNames: ["Off", ...animations.map((anim) => anim.name)],
    });
  });
    ...
    characteristics.forEach((c) => {
      if (c.type === "color") {
        if (c.value.type === "static") {
          ...
          c.value.parameters.colorFrom = Color.fromObject(
            c.value.parameters.colorFrom
          );
          c.value.parameters.colorTo = Color.fromObject(
            c.value.parameters.colorTo
          );
        }
      }
    });
      ...
      if (!currentCharacteristics[animation.name]) {
        currentCharacteristics[animation.name] = animation.options.map((o) => {
          ...
          if (o.type === "color") {
            characteristic = {
              type: "color",
              value: { type: "static", color: o.default },
            };
