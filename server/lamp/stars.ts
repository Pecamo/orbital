...
export default class StarsAnimation implements LampAnimation<[ColorOption, ColorOption, NumberOption, NumberOption, NumberOption]> {
    public name = "Stars";
    public options: [ColorOption, ColorOption, NumberOption, NumberOption, NumberOption] = [
        { name: "Color 1", type: "color", default: HtmlColors.cyan },
        { name: "Color 2", type: "color", default: HtmlColors.magenta },
        { name: "Star lifetime", type: "number", default: 120, min: 0, max: 600, step: 10, display: "range" },
        { name: "New stars per second", type: "number", default: 60, min: 0, max: 600, step: 10, display: "range" },
        { name: "Star width", type: "number", default: 1, min: 1, max: 10, step: 1, display: "input" },
    ];

    private readonly stars: Array<{ color: Color, life: number }> = [];
    private newStarIncrement: number = 0;
    ...
    public animate(t, display, options) {
        ...
        while (this.newStarIncrement >= LAMP_FPS) {
            ...
            for (let starPixelI = 0; starPixelI < starWidth; starPixelI++) {
                ...
                const pixelTranslation = Math.floor((starPixelI + 1) / 2) * pixelTranslationDirection;
                const starPixel = newStarCenter + pixelTranslation;
                this.stars[normalize(starPixel)] = { color: newColor, life: starLifetime };
            }
