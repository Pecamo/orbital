...
export class BattleRoyale {
  ...
  public static tick(gameState: GameState, display: Display) {
    Line.simplifyArray(gameState.battleRoyale.deathLines).forEach((line) =>
      display.drawLine(line, this.DEATH_COLOR)
    );
    Line.simplifyArray(gameState.battleRoyale.warnLines).forEach((line) =>
      display.drawLine(line, this.WARN_COLOR)
    );
  }
  ...
  public static nextState(
    ...
    if (
      gameState.turnNb >=
      gameState.battleRoyale.lastExpansion + this.EXPANSION_DELAY
    ) {
      ...
      if (warnLines.length === 0 && deathLines.length === 0) {
        ...
        nextState.battleRoyale.warnLines = [
          new Line(stageSize, initX, initX + this.EXPANSION_SIZE),
        ];
      } else {
        // New Death Lines
        warnLines.forEach((warnLine) => {
          nextState.battleRoyale.deathLines.push(
            new Line(stageSize, warnLine.from, warnLine.to)
          );
        });

        nextState.battleRoyale.deathLines = Line.simplifyArray(
          nextState.battleRoyale.deathLines
        );
        ...
        nextState.battleRoyale.deathLines.forEach((deathLine) => {
          nextState.battleRoyale.warnLines.push(
            new Line(
              stageSize,
              deathLine.to,
              deathLine.to + this.EXPANSION_SIZE
            )
          );
          nextState.battleRoyale.warnLines.push(
            new Line(
              stageSize,
              deathLine.from - this.EXPANSION_SIZE,
              deathLine.from
            )
          );
        });

        nextState.battleRoyale.warnLines = Line.simplifyArray(
          nextState.battleRoyale.warnLines
        );
      }
    }
