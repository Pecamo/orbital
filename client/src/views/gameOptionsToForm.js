...
function generateInputBoolean(gameOption, name) {
  ...
  return `
        <label class="name" for="${name}">${formatName(
    name
  )} <small>(default: ${gameOption.default})</small>:</label>
...
function generateInputEnum(gameOption, name) {
  let html = `
        <div class="name">${formatName(name)} <small>(default: ${
    gameOption.default
  })</small>:</div>
        <div class="radio-inputs">
    `;

  html += gameOption.options
    .map((option) => {
...
function generateInputNumber(gameOption, name) {
  ...
  return `
        <label class="name" for="${name}">${formatName(
    name
  )} <small>(${min}-${max}, default: ${gameOption.default})</small>:</label>
        <input type="number" name="${name}" min="${min}" max="${max}" value="${
    gameOption.value
  }" size="5" onchange="updateValue(this, parseFloat(this.value))">
    `;
}
